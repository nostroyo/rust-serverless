...
use rust_serverless::common::car::{Car, Driver};
...
async fn put_item(
    ...
    let driver_name = match request.body() {
        Body::Empty => "".to_string(),
        Body::Text(body) => body.clone(),
        Body::Binary(body) => String::from_utf8_lossy(body).to_string(),
    };

    let car = Car::new(Some(Driver::new(&driver_name)));

    // Put the item in the DynamoDB table
    let res = client
        .put_item()
        .table_name(table_name)
        .item("id", AttributeValue::S(car.pk.to_string()))
        .item("payload", AttributeValue::S(serde_json::to_string(&car)?))
        .send()
        .await;
    ...
    match res {
        Ok(_) => Ok(Response::builder()
            .status(200)
            .body(format!("item saved {}", car.pk.to_string()).into())?),
